...
/*   Updated: 2025/05/30 15:20:30 by qhatahet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_last_status(t_shell *shell, pid_t last_pid)
{
	int	status;
...
void	execute_multiple(t_token *tokens, t_shell *shell)
{
	...
	while (++shell->exe->index < shell->exe->pipes_count + 1)
		get_last_status(shell, last_pid);
	clean_execute_multiple(shell);
}
